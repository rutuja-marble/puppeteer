...
async function scrapeReviews(page, url) {
  ...
  while (hasMoreReviews && retryCount < maxRetries) {
    ...
    try {
      await page.waitForSelector('.jdgm-rev', { timeout: 0 });
    ...
    fs.writeFileSync('milky_mama_unique_reviews1.csv', csv1);
    fs.writeFileSync('milky_mama_all_reviews1.csv', csv2);
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync('milky_mama_reviews1.csv', csv);
